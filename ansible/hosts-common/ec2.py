...
class Ec2Inventory(object):
    ...
    def add_instance(self, instance, region):
        ...
        # Added by fdupoux: add EC2 instance to its primary group
        primary_group = getattr(instance, 'tags').get("ansible_host_group", None)
        if primary_group != None:
            self.push(self.inventory, primary_group, hostname)
    ...
    def get_host_info_dict_from_instance(self, instance):
        ...
        # Added by fdupoux so ansible can use the external or internal IP when needed
        ec2_ip_address = instance_vars['ec2_ip_address']
        if instance_vars.get("ec2_tag_internal_only", "false") == "true":
            ansible_ssh_host = instance_vars['ec2_private_ip_address']
        else:
            ansible_ssh_host = ec2_ip_address
        instance_vars['ansible_ssh_host'] = ansible_ssh_host
        # Added by fdupoux: define internal_ip
        instance_vars['internal_ip'] = instance_vars['ec2_private_ip_address']
        # Added by fdupoux: define service_hostname
        instance_fqdn = instance_vars['ec2_tag_Name']
        if instance_fqdn != None:
            instance_vars['service_hostname'] = instance_fqdn.split(".")[0] + '.dupoux.int'
        # Added by fdupoux: define external_ip when the instance has a public address
        if (ec2_ip_address != None) and (ec2_ip_address != ""):
            instance_vars['external_ip'] = ec2_ip_address
